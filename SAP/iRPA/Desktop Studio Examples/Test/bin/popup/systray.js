...
var _isAutoRestartEnabled = false;
var _currentPageId = "";
...
function initializeSystray() {	
	...
	$(window).trigger("focus");//needed because "blur" event does not trigger until the popup has acquired focus
	_initTrialModeAlert();
	_fixButtonStyles();
	_initHelpButtons();
	_addDialogBoxes();
...
function _addDialogBoxes() {
	_initRestartOnProjectCustomModal();
	...
	_initAutoProjectSwitchModeCustomModal();
	_initManualProjectSwitchModeCustomModal()
	_initSwitchToDefaultProjectCustomModal();
}
...
function _initRestartOnProjectCustomModal() {	
	$("#containerProjects").append(_getConfirmationDialogBoxHTML('restartOnProject', GLOBAL.labels.systray.switchProjectText, 'OK', GLOBAL.labels.systray.cancel));
	$("#restartOnProject_modal_OK").click(_onSwitchProject);
}

function _initSwitchToDefaultProjectCustomModal() {	
	$("#containerProjects").append(_getConfirmationDialogBoxHTML('switchDefaultProject', GLOBAL.labels.systray.restartOnDefaultProject, 'OK', GLOBAL.labels.systray.cancel));
	$("#switchDefaultProject_modal_OK").click(_onDefaultModeItemDialogOKClick);
}

function _initManualProjectSwitchModeCustomModal() {
	$("#projectModeSelect").after(_getConfirmationDialogBoxHTML('manualSwitchProject', GLOBAL.labels.systray.switchToManualProjectSwitchMode, 'OK', GLOBAL.labels.systray.cancel));
	$("#manualSwitchProject_modal_OK").click(_onManualSwitchProjectModeDialogOKClick);
	$('#manualSwitchProject_modal').on('hidden.bs.modal', function() {POPUPS.Systray.projectModeSelect.set(_previousSwitchProjectMode)});
}

function _initAutoProjectSwitchModeCustomModal() {
	$("#projectModeSelect").after(_getConfirmationDialogBoxHTML('autoSwitchProject', GLOBAL.labels.systray.switchToAutoProjectSwitchMode, 'OK', GLOBAL.labels.systray.cancel));
	$("#autoSwitchProject_modal_OK").click(_onAutoSwitchProjectModeDialogOKClick);
...
function _onSwitchProject() {
	var obj = _findObjectInArrayById(_projectList, _selectedProjectId, "packageVersionUid");
	var sProjectPackageUid = obj.packageUid;

	var sEnvironmentUid = '';
	if (obj.environmentUid) sEnvironmentUid = _findObjectInArrayById(_projectList, _selectedProjectId, "packageVersionUid").environmentUid;
	else sEnvironmentUid = _findObjectInArrayById(_projectList, _selectedProjectId, "packageVersionUid").environmentClassifier;
...
function _onClickTenant(ev) {
	...
	_selectedTenantName = _findObjectInArrayById(_tenantList, _selectedTenantId, "id").name;
	_selectedTenantUrl = _findObjectInArrayById(_tenantList, _selectedTenantId, "id").url;	
	...
	if (!_findObjectInArrayById(_tenantList, _selectedTenantId, "id").active) {
...
function onSuccessAddEditTenant(sTenantId, sTenantName, sTenantUrl) {
	...
	if (!foundTenant) {
		...
		_editObjectInArrayById(_tenantList,sTenantId, {
			...
			name: sTenantName,
			url: sTenantUrl,
			active: _findObjectInArrayById(_tenantList, sTenantId, "id").active
		});
	}
...
function _onSaveSettings() {
	if (_currentLanguage !== POPUPS.Systray.language.get()) {
		_currentLanguage = POPUPS.Systray.language.get();
		sendEventToSDK("SET_LANGUAGE", {
			language: _currentLanguage
		});
	}
	if (_isStudioMode && _currentEnvUid !== POPUPS.Systray.envList.get()) {
		_currentEnvUid = POPUPS.Systray.envList.get();
		sendEventToSDK("SET_ENVIRONMENT", {
			envUid: _currentEnvUid
		});
	}
	if (_isAutoRestartEnabled !== $("#chAutoRestart").prop('checked')) {
		_isAutoRestartEnabled = $("#chAutoRestart").prop('checked');
		sendEventToSDK("AUTO_RESTART_AGENT", {
			isAutoRestartEnabled: _isAutoRestartEnabled
		});
	}	
	_leaveSettingsPage();
}
...
function _onChangeInSettingsPage() {
	$("#btSettingsSave").prop('disabled', !_isSettingsHaveChanged());
...
function _onClickSettingsBackDialogBoxOKButton() {
	POPUPS.Systray.language.set(_currentLanguage);
	POPUPS.Systray.envList.set(_currentEnvUid);
	$("#chAutoRestart").prop('checked', _isAutoRestartEnabled);
	_leaveSettingsPage();
}
...
function _isSettingsHaveChanged() {
	return (_currentLanguage !== POPUPS.Systray.language.get()) || (_isAutoRestartEnabled !== $("#chAutoRestart").prop('checked')) || (_isStudioMode && _currentEnvUid !== POPUPS.Systray.envList.get());
}
	...
	if (inputProjectListLength > 0) {
		for (let i = 0; i < inputProjectListLength; i++) {
			...
			aProjectArray.push({
				...
				style: bIsProjectActive ? e.item.style.Cyan : e.item.style.None,
	...
	if (inputTenantListLength > 0) {
		for (let i = 0; i < inputTenantListLength; i++) {
			aTenantArray.push({
				...
				style: aInputTenantList[i].active ? e.item.style.Cyan : e.item.style.Grey,
...
/**
* To update job list in "Job Progress" page based on a json
* JSON Format : {<jobId> : ctx.jobClass, etc...}
* @param {*} oJobListMap Job List JSON
*/
function updateJobList(oJobListMap) {
	_oJobListMap = oJobListMap;
	var sTableCHtmlContent = '';
	for (var key in _oJobListMap) {
		var job = _oJobListMap[key];
		var bJobFailed = job.code && job.code !== "OK";		
		var bJobSuccessful = job.code && job.code === "OK";		
		sTableCHtmlContent = '<tr id="'+ key +'"><td style class="text-default ' + (bJobFailed ? 'agent-systray-job-progress-failed-row' : '') + (bJobSuccessful ? 'agent-systray-job-progress-success-row' : '') + '">' + job.status + '</td><td style class="text-default">' + (job.tsRun ? job.tsRun.substring(job.tsRun.indexOf(' ') + 1) : '') + '</td><td style class="text-default">' + (job.name || job.scenario) + '</td></tr>' + sTableCHtmlContent;
	}
	$("#tableJobProgress tbody").html(sTableCHtmlContent);//need to build a custom table with row id to have callback on row click event
	$("#tableJobProgress > tbody > tr").click(_onClickTableJobProgressRow);
}
...
function setIsAutoRestartEnabled(isAutoRestartEnabled) {
	_isAutoRestartEnabled = isAutoRestartEnabled;
	$("#chAutoRestart").prop('checked',_isAutoRestartEnabled);
}
